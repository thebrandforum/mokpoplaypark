...
export default function SafetyPage() {
  ...
  const loadSafetyData = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/admin/usage-info?section=safety')
      const result = await response.json()

      if (result.success) {
        setSafetyData(result.data)
      } else {
        console.error('안전수칙 데이터 로드 실패:', result.message)
      }
    } catch (error) {
      console.error('안전수칙 데이터 로드 중 오류:', error)
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <BaseLayout>
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="text-center">
            <div className="w-16 h-16 border-4 border-orange-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
            <p className="text-gray-600">안전수칙 정보를 불러오는 중...</p>
          </div>
        </div>
      </BaseLayout>
    )
          ...
          <div className="bg-white/90 px-4 sm:px-8 py-4 rounded inline-flex">
            ...
            <div className="grid grid-cols-1 md:flex md:items-center md:space-x-2">
              ...
              <div className="flex justify-center items-center mb-2 md:mb-0">
                ...
                <a href="/safety" className="text-orange-500 border-b-2 border-orange-500 pb-1 px-3 sm:px-4 text-sm sm:text-base whitespace-nowrap">
                  이용안전수칙
                </a>
        ...
        <div className="space-y-8 sm:space-y-10">
            ...
            <div key={section.id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
              <div className="flex flex-col lg:flex-row">
                ...
                <div className={`text-white p-6 sm:p-8 text-center flex flex-col justify-center items-center lg:w-48 ${
                  section.title.includes('이용자 안전수칙') || section.title.includes('안전수칙') ? 'bg-blue-500' :
                  section.title.includes('이용자의 책임') || section.title.includes('책임') ? 'bg-red-500' : 'bg-gray-500'
                }`}>
                  {(section.title.includes('이용자 안전수칙') || section.title.includes('안전수칙')) && <ShieldCheckIcon className="w-6 h-6 sm:w-8 sm:h-8 mb-2" />}
                  {(section.title.includes('이용자의 책임') || section.title.includes('책임')) && <ExclamationTriangleIcon className="w-6 h-6 sm:w-8 sm:h-8 mb-2" />}
          ...
          {(!safetyData?.sections || safetyData.sections.length === 0) && (
            <div className="text-center py-12 sm:py-20">
              <p className="text-gray-500 text-base sm:text-lg">안전수칙 정보가 없습니다.</p>
            </div>
          )}
        </div>
      </section>
